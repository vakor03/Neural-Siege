...
namespace _Project.Scripts.Core.PathCreation
{
    public class BacktrackingPathCreation : IPathCreationStrategy
    {
        ...
        private (Vector2Int start, Vector2Int end) GetStartAndFinish(Vector2Int gridSize)
        {
            while (true)
            {
                Vector2Int start = new Vector2Int(Random.Range(0, gridSize.x/2), Random.Range(0, gridSize.y/2));
                Vector2Int end = new Vector2Int(Random.Range(gridSize.x/2, gridSize.x), Random.Range(gridSize.y/2, gridSize.y));
